...
# Aria 적용 가이드 — Airbnb → 다크 모드 AI 아트 앱

> 위 Airbnb 디자인 시스템을 Aria 앱에 적용할 때의 변환 규칙.
> Airbnb 원본의 구조·패턴·철학은 유지하되, 색상·브랜딩·컨텍스트를 Aria에 맞게 변환한다.

## A1. 컬러 매핑

| Airbnb 원본 | Aria 변환 | 역할 |
|-------------|-----------|------|
| Canvas White `#ffffff` | Deep Black `#0D0D0D` | 페이지 배경 |
| Soft Cloud `#f7f7f7` | Surface `#1A1A1A` | 카드/서피스 배경 |
| Hairline Gray `#dddddd` | Dark Border `#333333` | 구분선, 카드 테두리 |
| Ink Black `#222222` | Pure White `#FFFFFF` | 제목/본문 텍스트 |
| Ash Gray `#6a6a6a` | Muted `#A3A3A3` | 보조 텍스트 |
| Rausch `#ff385c` | Violet `#8B5CF6` | 프라이머리 CTA, 활성 탭 |
| Deep Rausch `#e00b41` | Deep Violet `#7C3AED` | 눌림 상태 |
| Info Blue `#428bff` | 유지 `#428bff` | 정보 링크 |
| Error Red `#c13515` | 유지 `#EF4444` | 에러 상태 |
| — (좋아요 하트) | Heart Red `#EF4444` | Airbnb와 동일하게 빨간 하트 유지 |

## A2. 컴포넌트 변환

### 버튼
```
Primary CTA:   bg-[#8B5CF6] text-white rounded-xl py-3.5 px-6
               pressed: scale(0.96), bg-[#7C3AED]
               focus ring: 0 0 0 2px #8B5CF6

Secondary:     bg-transparent border border-[#333] text-white rounded-xl py-3.5 px-6
               pressed: bg-[#1A1A1A]

Icon Circle:   bg-[#1A1A1A] rounded-full 36-44px
               icon stroke: #E5E5E5
               pressed: scale(0.92)

Disabled:      opacity-0.4, 터치 비활성
```

### 카드 (작품 카드 — Airbnb Listing Card 변환)
```
배경: #1A1A1A (Soft Cloud → Surface)
모서리: rounded-xl (14px)
그림자: 없음 (Airbnb 동일)
이미지: 4:3 full-bleed, 상단 rounded-t-xl
하단 메타: px-3 py-3
  - 작가명: 14px 600 #FFFFFF
  - 작품 제목: 14px 500 #E5E5E5
  - 좋아요 수: 12px 400 #A3A3A3 + 하트 아이콘
```

### 입력 필드
```
bg-[#1A1A1A] border border-[#333] rounded-xl px-4 py-3.5
포커스: border-[#8B5CF6]
에러: border-[#EF4444]
placeholder: #737373
텍스트: #FFFFFF
...
아이콘: 비활성 #737373 / 활성 #8B5CF6
라벨: 12px, 아이콘 아래
높이: 83px (safe area 포함)
```

### 검색 바
```
Airbnb 3-segment pill → Aria 단일 검색 바
bg-[#1A1A1A] border border-[#333] rounded-2xl px-4 py-3
아이콘: 돋보기 #737373 좌측
placeholder: "작품, 작가 검색" #737373
포커스: border-[#8B5CF6]
```
...
| Airbnb | Aria |
|--------|------|
| Airbnb Cereal VF | 시스템 폰트 (SF Pro / Roboto) |
| 500 기본 굵기 | 400 Regular (시스템 폰트는 400이 표준) |
| 600 강조 | 600 SemiBold 유지 |
| 700 제목 | 700 Bold 유지 |
| Ink Black `#222222` | Pure White `#FFFFFF` |

작품 제목에만 선택적으로 Playfair Display (serif) 적용 가능.
...
|------|--------|------|
| 피드 카드 비율 | 4:3 | **4:3** (동일) |
| 그리드 비율 | — | **1:1** (포트폴리오 그리드) |
| 상세 뷰 | 5장 그리드 | 단일 이미지 전체 보기 (최대 5장 스와이프) |
| 아바타 | 50% 원형 | **50% 원형** (동일) |
| 모서리 | 14-20px | **14px** 통일 |
| 로딩 | 블러 → 선명 | **블러 → 선명** (동일) |
| 다크 모드 대비 | — | 이미지 주변 다크 배경이 작품 색감을 살려줌 |

## A5. 다크 모드 전용 원칙

1. **그림자 대신 밝기 차이로 깊이 표현** — `#0D0D0D` → `#1A1A1A` → `#262626` 순으로 올라갈수록 밝아짐
2. **Elevation = 밝기** — Airbnb의 box-shadow 3-layer 시스템은 다크 모드에서 효과가 약하므로, surface 색상 차이로 대체
3. **텍스트 대비 최소 4.5:1** — WCAG AA 기준 준수. `#A3A3A3` on `#0D0D0D` = 약 7:1 (통과)
4. **이미지가 가장 밝은 요소** — 다크 배경 위에서 작품 이미지가 자연스럽게 주인공이 됨
5. **포인트 컬러 절제** — Violet `#8B5CF6`는 CTA와 활성 탭에만. 한 화면에 2개 이상의 바이올렛 요소가 보이면 하나를 중립색으로

## A6. 애니메이션

Airbnb 원본에서 애니메이션 사양이 문서화되지 않았으므로 Aria 자체 정의:

| 효과 | 속성 | 시간 | Easing |
|------|------|------|--------|
| 화면 전환 | 우→좌 슬라이드 | 300ms | ease-out |
| 좋아요 하트 | scale 1→1.3→1 | 200ms | spring |
| 카드 터치 | scale 1→0.98 | 100ms | ease-in |
| 바텀시트 | translateY + opacity | 250ms | ease-out |
| 이미지 로딩 | blur→clear | 400ms | ease-in-out |
| 탭 전환 | opacity crossfade | 200ms | ease-in-out |

**금지**: 글로우, 네온, 그라데이션 오브, pulse, 무한 회전, 3D perspective.

## A7. 아이콘

- 라이브러리: `@expo/vector-icons` → **Feather** 세트
- Airbnb의 3D 렌더 아이콘은 사용하지 않음 (라인 아이콘으로 통일)
- 크기: 24px (기본) / 20px (카드 내) / 28px (탭 바)
- 색상: 텍스트 계층 규칙 동일 적용
- 둥근 배경으로 감싸지 않음 (Airbnb의 원형 아이콘 버튼은 예외)
