...
describe('toggleBookmark', () => {
  it('북마크가 없으면 생성하고 bookmarked: true를 반환한다', async () => {
    ...
    const result = await toggleBookmark('user1', 'art-1');
  ...
  it('북마크가 이미 있으면 삭제하고 bookmarked: false를 반환한다', async () => {
    mockRunTransaction.mockImplementation(
      async (_db: unknown, fn: Function) => {
        const tx = {
  ...
  it('Transaction으로 bookmarks 문서와 bookmarksCount를 동시에 처리한다', async () => {
    let txActions: { set?: boolean; update?: boolean } = {};
    mockRunTransaction.mockImplementation(
      async (_db: unknown, fn: Function) => {
        const tx = {
          get: jest.fn().mockResolvedValue({ exists: () => false }),
    ...
    await toggleBookmark('user1', 'art-1');
...
describe('checkBookmarked', () => {
  it('북마크가 있으면 true를 반환한다', async () => {
    mockGetDoc.mockResolvedValue({ exists: () => true });

    const result = await checkBookmarked('user1', 'art-1');
...
describe('getBookmarkedArtworks', () => {
  const now = new Date();
  const firestoreTimestamp = { toDate: () => now };

  it('저장한 작품 목록을 반환한다', async () => {
    ...
    const result = await getBookmarkedArtworks('user1');
  ...
  it('저장한 작품이 삭제된 경우 목록에서 제외한다', async () => {
    mockGetDocs.mockResolvedValueOnce({
      docs: [
        {
          id: 'user1_art-1',
