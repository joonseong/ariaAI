...
import { getBookmarkedArtworks } from '@/services/bookmarks';
...
export function useSavedArtworks() {
  ...
  const load = useCallback(async () => {
    ...
    const result = await getBookmarkedArtworks(user.id);
  ...
  const loadMore = useCallback(async () => {
    ...
    const result = await getBookmarkedArtworks(user.id, cursorRef.current);
